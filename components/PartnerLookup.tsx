          ...
          <div className="bg-blue-50 rounded-xl p-4 border border-blue-200">
            <div className="flex items-center justify-between">
              <p className="text-blue-800">
                <span className="font-semibold">Chào mừng!</span> Bạn đã đăng
                nhập thành công.
              </p>
              <button
                ...
                <div className="divide-y divide-gray-200">
                  {results.length === 0 && !isLoadingResults ? (
                    <div className="p-8 text-center text-gray-500">
                      ...
                      <p className="text-sm">
                        Thử thay đổi từ khóa tìm kiếm hoặc khoảng thời gian
                      </p>
                    </div>
                  ) : (
                    results.map((result, index) => {
                      const isLast = index === results.length - 1;
                      const { displayName, colorClass } = getStatusInfo(
                        result.state
                      );
        ...
        {!isLoggedIn ? (
          ...
          <ul className="text-blue-700 space-y-2">
            <li className="flex items-start space-x-2">
              <span className="text-blue-600 mt-1">•</span>
              <span>
                Sử dụng ô tìm kiếm để tìm theo tên bệnh nhân hoặc mã SID
              </span>
            </li>
            <li className="flex items-start space-x-2">
              <span className="text-blue-600 mt-1">•</span>
